--[[
 * ReaScript Name: GU-on Set Time Before Snap Offset
 * Author: GU-on
 * Licence: GPL v3
 * REAPER: 6.27
 * Version: 1.0
...
local r = reaper
local ctx = r.ImGui_CreateContext('Set Time Before Snap Offset', 300, 60)
...
function Main(x)
	...
	for i, item in ipairs(init_sel_items) do
		...
		r.ApplyNudge(0, 0, 1, 1, item_snapoffset, false, 0)
		r.SetMediaItemInfo_Value(item, "D_SNAPOFFSET", 0)
		
		r.ApplyNudge(0, 0, 1, 1, x, true, 0)
		r.SetMediaItemInfo_Value(item, "D_SNAPOFFSET", x)
		reaper.SetMediaItemSelected( item, false )
	end
...
if count_sel_items > 0 then
	...
	function loop()
		...
		if (r.ImGui_Button(ctx, 'Apply') or r.ImGui_IsKeyPressed(ctx, 13)) then
			...
			r.Undo_EndBlock("Set time before snap offset", - 1) -- End of the undo block. 
			r.UpdateArrange()
			r.PreventUIRefresh(-1)
